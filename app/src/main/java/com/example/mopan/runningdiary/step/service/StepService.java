package com.example.mopan.runningdiary.step.service;
...
import com.example.mopan.runningdiary.step.UpdateUiCallBack;
import com.example.mopan.runningdiary.step.accelerometer.StepCount;
import com.example.mopan.runningdiary.step.accelerometer.StepValuePassListener;
...
public class StepService extends Service implements SensorEventListener {
     ...
     * 默认为25秒进行一次存储
     */
    private static int duration = 25 * 1000;
    /**
     * 当前的日期
     */
    ...
    public void onSensorChanged(SensorEvent event) {
        if (stepSensorType == Sensor.TYPE_STEP_COUNTER) {
            ...
            if (!hasRecord) {
                ...
                Logger.e("CurrentStep:"+CURRENT_STEP);
                //记录最后一次APP打开到现在的总步数
            ...
            Toast.makeText(getApplicationContext(),"rrrrrr",Toast.LENGTH_SHORT).show();
            Logger.e("CurrentStep:"+CURRENT_STEP);
        }
...
//        }
            ...
            if(recordtimes == 11){
                List<AverageStepsData> averageStepslist = DbUtils.getQueryAll(AverageStepsData.class);
                List<StepData> stepDatas = DbUtils.getQueryAll(StepData.class);
                long Allsteps = 0;

                for (int i = 0;i < stepDatas.size(); i++){
                    Allsteps += Long.parseLong(stepDatas.get(i).getStep());
                }
                // average steps
                long average = Allsteps/stepDatas.size();

                if (averageStepslist.size() == 0 || averageStepslist.isEmpty()) {

                    AverageStepsData averageStep = new AverageStepsData();
                    averageStep.setMonth(CURRENT_DATE);
                    averageStep.setStep(average + "");
                    DbUtils.insert(averageStep);

                }
                else if (averageStepslist.size() == 1) {
                    AverageStepsData averageStep = new AverageStepsData();
                    averageStep.setMonth(CURRENT_DATE);
                    averageStep.setStep(average + "");
                    DbUtils.update(averageStep);
                }
